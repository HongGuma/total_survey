...
const QuestionForm = ({index, caId}) => {
    ...
    async function LoadList(caID){
        const res = await axios.post(process.env.REACT_APP_API+"/survey/question",{index:index,caID:caID});
        return res.data;
    }
    return(
        <div className="review-question">
            <div>
                {queList != null && queList.map((item,idx)=>(
                    <div key={idx}>
                        {item.q_type > -1 && item.question}
                        {item.q_type == 0 && <QuestionType00 index={index} qType={item.q_type} qID={item.q_id} subID={item.sub_id}/>}
                        {/*<AnswerType01 index={index} qID={item.q_id} qSubID={item.sub_id}/>*/}
                    </div>
                ))}
            </div>
        </div>
