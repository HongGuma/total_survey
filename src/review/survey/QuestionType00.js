...
const QuestionType00 = ({index,qType,qID,subID}) => {
    ...
    async function LoadList(){
        const res = await axios.post(process.env.REACT_APP_API+'/survey/answer',{index:index,qid:qID,subid:subID});
        return res.data;
    }
    return(
        <div className="default-answer">
            {saList != null && saList.map((item,idx)=>(
                <div key={idx}>
                    <label><input type="radio" name={item.sa_tag}/>{item.sa_content.replace(/\d{1,3}./g,'')}</label>
                </div>
            ))}
        </div>
    )
