...
const CheckSurvey = ({match}) => {
    ...
    async function exportCategory(){
        const res = await axios.post(process.env.REACT_APP_API+"/export/category",{fileName:fileName,index:index,sheet:0});
        if(res.data == '-1'){
    ...
    async function exportQuestion(){
        const res = await axios.post(process.env.REACT_APP_API+"/export/question",{fileName:fileName,index:index,sheet:0});
        if(res.data == '-1'){
            alert('에러 발생! 엑셀파일에 문제가 없는지 확인해주세요.');
        }else if(res.data == '-2'){
    ...
    async function exportAnswer(){
        const res = await axios.post(process.env.REACT_APP_API+"/export/answer",{fileName:fileName,index:index,sheet:0});
        if(res.data == '-1'){
            alert('에러 발생! 엑셀파일에 문제가 없는지 확인해주세요.')
        }else if(res.data == '-2'){
    ...
    async function saveData(){
        const res = await axios.post(process.env.REACT_APP_API+"/export/save",{index:index});
        if(res.data == '-1'){
    ...
    async function ReUpload(){
        ...
        const res = await axios.post(process.env.REACT_APP_API+"/file/reUpload",formData,{headers : {"Content-Type": "multipart/form-data"}});
        if(res.data == '-1'){
    ...
    return(
        <div className="check-survey">
            <div className="sect-01-top">
                <div><button onClick={MovePrev}>뒤로가기</button></div>
                <div>선택한 파일 : {fileName}</div>
                <div className="cont-03">error log</div>
            </div>
            <div className="sect-02">
                <div>엑셀파일 다시 올리기 : <input type="file" onChange={e=>FileInputHandler(e)} accept=".xlsx"/> <button onClick={ReUpload}>upload</button></div>
            </div>
            <div className="sect-03">
                ...
                {answerData != null && <div className="cont-02"><button onClick={saveData}>저장하기</button></div>}
            </div>
            <div className="sect-04">
                {categoryData != null && <CheckCategory data = {categoryData} qList={questionData} saList={answerData}/>}
            </div>
        </div>
    )
}
