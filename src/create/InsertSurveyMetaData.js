...
const InsertSurveyMetaData = ({match}) => {
    ...
    async function SaveData(){
        console.log('click');
        const res = await axios.post(process.env.REACT_APP_API+'/insert/metaData',{inputValue:inputValue});
        if(res.data == '-1'){
    ...
    async function loadIndex(){
        const res = await axios.post(process.env.REACT_APP_API+'/file/index',{value:inputValue});
        return res.data;
    }
    ...
    return(
        <div className="insert-meta-data">
            ...
            <div className="sect-02">
                <div className="sect-02-cont-01">메타 데이터 입력 (※ 메타 데이터는 수정이 불가합니다.)</div>
                <div className="sect-02-cont-02" onChange={e=>InsertValue(e)}>
                    <div className="cont-01">
                        <p>설문지 이름</p>
                        <p><input type="text" name="sr_name"/></p>
                    </div>
                    <div className="cont-01">
                        <p>설문지 버전</p>
                        <p><input type="text" name="sr_ver"/></p>
                    </div>
                    <div className="cont-01">
                        <p>설문지 제작자</p>
                        <p><input type="text" name="sr_writer"/></p>
                    </div>
                    <div className="cont-01">
                        <p>첫 생성 날짜</p>
                        <p><input type="text" name="create_dt" placeholder="yyyy.mm.dd"/></p>
                    </div>
                    <div className="cont-01">
                        <p>설문지 종류</p>
                        <p>
