...
const CreateMain = () => {
  ...
  return (
    <div className="create-main">
      <div>
        <button style={{ float: "left" }} onClick={MoveBack}>
          뒤로가기
        </button>
      </div>
      <h1>설문지 제작하기</h1>
...
const FileList = () => {
  ...
  async function FileNameChangeHandler() {
    const res = await axios.post(
      process.env.REACT_APP_API + "/file/changeName"
    );
  }
  ...
  async function FileDeleteHandler() {
    const res = await axios.post(process.env.REACT_APP_API + "/file/delete", {
      filename: selectFile,
      editor: "test1",
    });
    // console.log(res.data);
        ...
        <div className="sect-01">
          ...
          <ul>
            {fileList.map((item, idx) => (
              <li key={idx} onClick={() => FileClickHandler(item)}>
                {item}
              </li>
            ))}
          </ul>
          ...
          {logList != null &&
            logList.map((log, idx) => (
              <tr key={idx}>
                ...
                <td>{log.editor}</td>
                <td>
                  {log.reg_dt.slice(0, 4)}-{log.reg_dt.slice(4, 6)}-
                  {log.reg_dt.slice(6, 8)} {log.reg_dt.slice(8, 10)}:
                  {log.reg_dt.slice(10, 12)}:{log.reg_dt.slice(12, 14)}
                </td>
              </tr>
            ))}
        </tbody>
